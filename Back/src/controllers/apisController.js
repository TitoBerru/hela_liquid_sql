...
const apisController = {
  recipes: async function (req, res) {
    try{
     await db.Recetas.findAll().then(function (receta) {
  ...
  detail: async function (req, res) {
    try {
      ...
      // }
      // console.log('✅✅ linea 106 ************************************************* ',datosReceta)
      // Verificar si se encontró la receta
      if (receta) {
        // Enviar la receta al cliente en formato JSON
        // res.json(receta);
  ...
  deleteRecipe: async function (req, res) {
    ...
    try{
      ...
      if (recipeWithSales){
       return res.status(500).json('No se puede borrar la receta, ya que tiene ventas asociadas')
      }
      else{
    ...
    res.json("Receta Borrada");
      ...
      res
      .status(500)
      .json('Error al intentar borra receta - Error: ', error)
    }
  ...
  sales: async function (req, res) {
    try {
      ...
      res
      .status(500)
      .json("Error al consultar ventas - Error: ", error);
    }
  },
};
